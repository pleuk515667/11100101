...
-I think the problem we've been having with querying multiple bytes of sensor data is with the
  ...
  -NOT THE PROBLEM: I couldn't figure out the issue with reading multiple bytes.
    Try to work around this issue; I plan to talk to Prof. Li about it.
...
class Robot:
    ...
    #NEW!NEW!NEW!
    #A more refined way of changing the OI mode, eliminating
    # the need for timers before/after every OI change
    def changeMode(self, aMode):
        if(aMode == 'off'):
            self.writeCommand(7)
            self.checkMode(0)
        if(aMode == 'passive'):
    ...
    #NEW!NEW!NEW!
    #Used exclusively in the function 'changeMode' to ensure
    # that the OI mode has been changed after the command is sent
    def checkMode(self, n):
        timer = 0
        while timer!=10:
            time.sleep(0.15)
            self.writeCommand(142)
            self.writeCommand(35)
            if(self.readStatus()==n):
                return 1
            timer += 1
        return 0
    ...
    #The following eight functions  read sensor data from the robot, then return that
    # data in byte form.
